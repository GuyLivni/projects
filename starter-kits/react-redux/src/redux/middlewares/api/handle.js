...
function verifyHandlers(handlers, action) {
  Object.keys(handlers).forEach(key => {
    invariant(
      ...
      a valid key for a api-middleware handler. Valid keys are: ${Object.keys(
        VALID_KEYS
      )}
    `
    );
...
function handle(startingState, action, handlers) {
  ...
  if (lifecycle == null) {
    invariant(
      false,
      deline`
      You used api-middleware's \`handle(...)\` function on the action ${
        action.type
      }, however, it
      doesn't appear to be an action that was dispatched by api-middleware. This is likely an error.
    `
    );
